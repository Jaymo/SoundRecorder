...
public class Recorder implements OnCompletionListener, OnErrorListener {
	...
	public void renameSampleFile(String name) {
		if (mSampleFile != null && mState != RECORDING_STATE
			...
			if (!TextUtils.isEmpty(name)) {
				...
				File newFile = new File(mSampleFile.getParent() + "/" + name
						+ extension);
	...
	public void startRecording(int outputfileformat, String name,
			String extension, boolean highQuality, long maxFileSize) {
		stop();
		...
		RecorderService.startRecording(mContext, outputfileformat,
				mSampleFile.getAbsolutePath(), highQuality, maxFileSize);
